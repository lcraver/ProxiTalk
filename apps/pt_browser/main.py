...
class SimpleHTMLRenderer:
    ...
    def download_and_process_image(self, url, max_width=120, max_height=30):
        ...
        try:
            ...
            print(f"[FrogFind] Downloading image from: {url}")
            response = requests.get(url, timeout=10, headers={  # Increased timeout
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
            })
            response.raise_for_status()
            print(f"[FrogFind] Downloaded {len(response.content)} bytes")
            
            # Load image from bytes
            image_data = io.BytesIO(response.content)
            image = Image.open(image_data)
            print(f"[FrogFind] Opened image: {image.size} mode={image.mode}")
            
            # Convert to RGB if necessary
            if image.mode != 'RGB':
                image = image.convert('RGB')
                print(f"[FrogFind] Converted image to RGB")
            ...
            print(f"[FrogFind] Scaled image to: {new_width}x{new_height}")
            ...
            print(f"[FrogFind] Converted to monochrome")
