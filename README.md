...
```
ProxiTalk/
├── apps/           # Installed apps (auto-discovered by the launcher)
├── overlays/       # Background overlay apps (run on top of any app)
...
└── proxitalk.py    # Entry point
```

## Platform Paths (`config/paths.py` and `config/emulator/paths.py`)

ProxiTalk uses two path config files depending on where it is running:
- **`config/emulator/paths.py`** — used on Windows (emulator)
- **`config/paths.py`** — used on Linux (real hardware)
