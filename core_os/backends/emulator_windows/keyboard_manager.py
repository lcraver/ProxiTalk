...
class KeyboardManager:
    ...
    def inject(self, keycode: str, keystate: int) -> None:
        """Push a synthetic key event, e.g. from clicking the debug overlay's
        device map, into the same queue real key presses land in."""
        self._queue_event(KeyboardEvent(kind="key", keycode=keycode, keystate=keystate, timestamp=time.time()))
